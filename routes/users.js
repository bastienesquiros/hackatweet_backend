...
router.get('/getTweets', (req, res) => {
	User.findOne({ username: req.body.username }).then((data) => res.json({ result: true, userData: { username: data.username, firstname: data.firstname, token: data.token }, tweets: data.tweets.filter((obj) => obj.tweetContent.trim()) }));
});
