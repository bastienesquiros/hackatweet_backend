...
router.post('/addTweet', (req, res) => {
	...
	User.updateOne({ username: req.body.username }, { $push: { tweets: [{ date: Date.now(), tweetContent: req.body.tweetContent, likes: 0, hashtags: hashtags }] } }).then((data) => res.json({ result: true }));
});

router.get('/getTweetsByUserName/:username', (req, res) => {
	User.findOne({ username: req.params.username }).then((data) => res.json({ result: true, userData: { username: data.username, firstname: data.firstname, token: data.token }, tweets: data.tweets.filter((obj) => obj.tweetContent.trim()) }));
});
